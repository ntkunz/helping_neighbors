...
export default function App() {
  ...
  useEffect(() => {
    if (token) {
      ...
      makeApiCall("GET", "users/verify", null, true)
        .then((newUser) => setUser(newUser))
        .then(() => makeApiCall("GET", "users", null, true))
        .then((neighbors) => setNeighbors(neighbors.neighbors))
        .then(() => {
          setIsLoaded(true);
          setLoggedIn(true);
        })
      ...
      <div className="App__routes">
        <Routes>
          ...
          {!loggedIn && (
            <Route
              path="/signup"
              element={<NewUserPage setToken={setToken} />}
            />
          )}
          {loggedIn && (
            <Route
